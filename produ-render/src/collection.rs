...
These tuples should represent a set wi  thin a collection that will be used in a
...
use vulkano::descriptor::descriptor_set::{
    DescriptorSet, PersistentDescriptorSet, UnsafeDescriptorSet, UnsafeDescriptorSetLayout,
};
...
use crate::utils::{default_sampler, upload_data};
...
impl<T: SetUpload> Set<T> {
    ...
    pub fn upload(&mut self, device: Arc<Device>) {
        self.cached = self
            .data
            .upload(device, self.pipeline.clone(), self.set_idx);
    }
}
    ...
    fn upload(
        ...
        Arc::new(
            PersistentDescriptorSet::start(layout)
                .add_buffer(buffer)
                .expect(&format!("Panic adding 1st buffer at set idx "))
                .build()
                .expect(&format!("Panic finalizing set at set idx ")),
        )
    }
}
    ...
    fn upload(
        ...
        Arc::new(
            PersistentDescriptorSet::start(layout)
                ...
                .expect(&format!("Panic adding 1st image at set idx "))
                .build()
                .expect(&format!("Panic finalizing set at set idx ")),
        )
    }
    ...
    fn upload(
        ...
        Arc::new(
            PersistentDescriptorSet::start(layout)
                ...
                .expect(&format!("Panic adding 1st buffer at set idx "))
                .add_buffer(buffer2)
                .expect(&format!("Panic adding 2nd buffer at set idx "))
                .build()
                .expect(&format!("Panic finalizing set at set idx ")),
        )
    }
    ...
    fn upload(
        ...
        Arc::new(
            PersistentDescriptorSet::start(layout)
                ...
                .expect(&format!("Panic adding 1st image at set idx "))
                .add_sampled_image(self.1.clone(), sampler)
                .expect(&format!("Panic adding 2nd image at set idx "))
                .build()
                .expect(&format!("Panic finalizing set at set idx ")),
        )
    }
    ...
    fn upload(
        ...
        Arc::new(
            PersistentDescriptorSet::start(layout)
                ...
                .expect(&format!("Panic adding 2nd buffer at set idx "))
                .add_buffer(buffer3)
                .expect(&format!("Panic adding 3rd buffer at set idx "))
                .build()
                .expect(&format!("Panic finalizing set at set idx ")),
        )
    }
    ...
    fn upload(
        ...
        Arc::new(
            PersistentDescriptorSet::start(layout)
                ...
                .expect(&format!("Panic adding 3rd image at set idx "))
                .build()
                .expect(&format!("Panic finalizing set at set idx ")),
        )
    }
...
pub type Image = Arc<dyn ImageViewAbstract + Send + Sync>;

pub trait Data: Send + Sync + Clone + Copy + 'static {}
