...
impl PipelineCache {
    ...
    pub fn print_stats(&self) {
        let avg: f32 = self.stats.gen_times.iter().sum::<f32>()
            / (self.stats.gen_times.len() as f32)
            * 1_000.0;
        let percent =
...
impl PipelineSpec {
    pub fn concrete(
        ...
        // let shader_sys = ShaderSyzstem::load_from_file(device.clone(), &self.vs_path, &self.fs_path);
