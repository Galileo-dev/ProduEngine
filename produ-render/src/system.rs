...
use vulkano::render_pass::{AttachmentDesc, Framebuffer, FramebufferAbstract, RenderPass, Subpass};
...
pub struct System<'a> {
    ...
    cached_images: Option<HashMap<String, Arc<dyn ImageAccess + Send + Sync>>>,
    pub custom_images: HashMap<&'a str, Arc<dyn ImageAccess + Send + Sync>>,
    state: DrawState,
...
enum DrawState {
    ...
    Drawing {
        ...
        pass_idx: usize,
        images: HashMap<String, Arc<dyn ImageAccess + Send + Sync>>,
...
impl<'a> System<'a> {
    pub fn new(
        queue: Arc<Queue>,
        passes: Vec<Pass<'a>>,
        custom_images: HashMap<&'a str, Arc<dyn ImageAccess + Send + Sync>>,
        output_tag: &'a str,
    ) -> Self {
    ...
    pub fn start(&mut self, dest_image: Arc<dyn ImageAccess + Send + Sync>) {
    ...
    fn get_images(
        &mut self,
        dimensions: [u32; 2],
    ) -> HashMap<String, Arc<dyn ImageAccess + Send + Sync>> {
    ...
    match images.len() {
        ...
        1 => Arc::new(
            Framebuffer::start(render_pass)
                .add(ImageView::new(images[0].clone()).unwrap())
                .unwrap()
                .build()
                .unwrap(),
        ),
        2 => Arc::new(
            Framebuffer::start(render_pass)
                .add(ImageView::new(images[0].clone()).unwrap())
                .unwrap()
                .add(ImageView::new(images[1].clone()).unwrap())
                .unwrap()
                .build()
                .unwrap(),
        ),
        3 => Arc::new(
            Framebuffer::start(render_pass)
                ...
                .add(ImageView::new(images[1].clone()).unwrap())
                .unwrap()
                .add(ImageView::new(images[2].clone()).unwrap())
                .unwrap()
                .build()
                .unwrap(),
        ),
        4 => Arc::new(
            Framebuffer::start(render_pass)
                ...
                .add(ImageView::new(images[2].clone()).unwrap())
                .unwrap()
                .add(ImageView::new(images[3].clone()).unwrap())
                .unwrap()
                .build()
                .unwrap(),
        ),
...
fn images_for_passes<'a>(
    ...
    for pass in passes.iter() {
        ...
        let num_rpass_images = pass.render_pass.desc().attachments().len();
...
This means the image description for each tag cannot be determined from the render pass!",
        ...
        for (image_idx, &image_tag) in pass.images_created_tags.iter().enumerate() {
            let desc = pass.render_pass.desc().attachments()[image_idx];
            //             .expect(&format!(
            ...
            // Image tag: {}",
            //                 dimensions, image_idx, image_tag,
            //             ));
